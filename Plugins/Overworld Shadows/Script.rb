  ...
  class ShadowSprite
    ...
    def visible?
      ...
      WHITELIST.each do |str|
        return true if str.downcase.include?(@event.name.downcase) || comments.any? { |c| c.include?(str) }
      end

      BLACKLIST.each do |str|
        return false if str.downcase.include?(@event.name.downcase) || comments.any? { |c| c.include?(str) }
      end
