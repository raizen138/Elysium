...
class Battle
  ...
  def pbMegaEvolve(idxBattler)
    ...
    if battler.wild?
      ...
      when 2
        pbDisplay(_INTL("{1}'s mastery is enabling {1} to reach it's peak!", battler.pbThis))
      when 3
        pbDisplay(_INTL("{1}'s {2} radiates with a strange energy!", battler.pbThis, battler.itemName))
      else
