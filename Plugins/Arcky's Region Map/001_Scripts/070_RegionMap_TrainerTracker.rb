        ...
        unless map.nil?
          unless pbMapInterpreter.get_self.nil?
            ...
            if map.events[eventID].name[/trainer/i]
              ...
              district = regionMap.getDistrict((map.metadata).town_map_position)
              $PokemonGlobal.trainerTracker[district] ||= { :total => 0 }
              $PokemonGlobal.trainerTracker[district][:maps] ||= {}
              $PokemonGlobal.trainerTracker[district][:maps][map.map_id] ||= {:defeated => 0}
              $PokemonGlobal.trainerTracker[district][:maps][map.map_id][eventID] ||= { :defeated => 0 }
              unless $PokemonGlobal.trainerTracker[district][:maps][map.map_id][eventID][:defeated] != 0
                $PokemonGlobal.trainerTracker[district][:maps][map.map_id][eventID][:defeated] += $PokemonGlobal.trainerTracker[:trainers]
                $PokemonGlobal.trainerTracker[district][:total] += $PokemonGlobal.trainerTracker[:trainers]
                $PokemonGlobal.trainerTracker[district][:maps][map.map_id][:defeated] += $PokemonGlobal.trainerTracker[:trainers]
              end
            end 
          end
        end 
