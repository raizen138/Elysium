...
class PokemonRegionMap_Scene
  ...
  def pbMapScene
    ...
    # @zoomStages = [0.5, 1.0, 2.0]
    @zoomLevel = 1.0 #@zoomStages[1]
    ...
    loop do
      ...
      # for v2.7.0
=begin
      if Input.trigger?(Input::CTRL) && @mode == 0
        if !@zoom
          @zoom = true
          @sprites["modeName"].bitmap.clear
          pbDrawTextPositions(
        ...
        if Input.trigger?(Input::JUMPUP)
          if @zoomLevel == @zoomStages[0]
            @zoomLevel = @zoomStages[1]
          elsif @zoomLevel == @zoomStages[1]
            @zoomLevel = @zoomStages[2]
          end
        elsif Input.trigger?(Input::JUMPDOWN)
          if @zoomLevel == @zoomStages[2]
            @zoomLevel = @zoomStages[1]
          elsif @zoomLevel == @zoomStages[1]
            @zoomLevel = @zoomStages[0]
          end
        end
        @spritesMap.each do |key, value|
          @spritesMap[key].zoom_x = @zoomLevel
          @spritesMap[key].zoom_y = @zoomLevel
        end
        @sprites["cursor"].zoom_x = @sprites["cursor"].zoom_y = @zoomLevel
      end
=end
      updateArrows
  ...
  def getDirectionInput(ox, oy, mox, moy)
    case Input.dir8
    when 1, 2, 3
      oy = 1 * @zoomLevel  if @sprites["cursor"].y < @limitCursor[:maxY]
      moy = -1 * @zoomLevel if @spritesMap["map"].y > -1 * (@mapHeight - (Graphics.height - BEHIND_UI[3])) && oy == 0
    when 7, 8, 9
      oy = -1 * @zoomLevel if @sprites["cursor"].y > @limitCursor[:minY]
      moy = 1 * @zoomLevel if @spritesMap["map"].y < 0 && oy == 0
    end
    case Input.dir8
    when 1, 4, 7
      ox = -1 * @zoomLevel if @sprites["cursor"].x > @limitCursor[:minX]
      mox = 1 * @zoomLevel if @spritesMap["map"].x < 0 && ox == 0
    when 3, 6, 9
      ox = 1 * @zoomLevel if @sprites["cursor"].x < @limitCursor[:maxX]
      mox = -1 * @zoomLevel if @spritesMap["map"].x > -1 * (@mapWidth - (Graphics.width - BEHIND_UI[1])) && ox == 0
    end
