...
class Player < Trainer
    ...
    class SocialLinks
        ...
        def pbGainLinkBond(profile_id, val = 1, silent: false)
            ...
            @links[profile_id].bond = SocialLinkSettings::BOND_LEVEL_MAX if @links[profile_id].bond > SocialLinkSettings::BOND_LEVEL_MAX
            if old_val != @links[profile_id].bond
                unless silent
                    if @links[profile_id].bond == SocialLinkSettings::BOND_LEVEL_MAX && SocialLinkSettings::SHOW_MAXED_BOND_MESSAGE
        ...
        def pbSetLinkBond(profile_id, val = 1, silent: false)
            ...
            old_val = @links[profile_id].bond
            @links[profile_id].bond = val
            @links[profile_id].bond.clamp(0, SocialLinkSettings::BOND_LEVEL_MAX)
            if old_val != @links[profile_id].bond
        ...
        def pbLinkBondMaxed?(profile_id)
            profile_id = pbSanitizeProfileID(profile_id)
            return false if profile_id.nil?
            return false if !@links[profile_id] || @links[profile_id].bond < SocialLinkSettings::BOND_LEVEL_MAX
            return true
        end
